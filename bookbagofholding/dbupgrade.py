...
from bookbagofholding.common import restartJobs, pwd_generator
from bookbagofholding.formatter import plural, makeUnicode, makeBytestr, md5_utf8
from bookbagofholding.importer import addAuthorToDB, update_totals
...
def upgrade_needed():
    ...
    # 48 add Role, ApiKey, LastLogin, CreatedAt, PasswordChangedAt, PasswordAlgorithm to users table
...
def dbupgrade(db_current_version):
    with open(os.path.join(bookbagofholding.CONFIG['LOGDIR'], 'dbupgrade.log'), 'a') as upgradelog:
        ...
        try:
            ...
            if db_version < db_current_version:
                myDB = database.DBConnection()
                if db_version:
                    bookbagofholding.UPDATE_MSG = 'Updating database to version %s, current version is %s' % (
                    ...
                    bookbagofholding.UPDATE_MSG = 'Creating new database, version %s' % db_version
                    upgradelog.write("%s v0: %s\n" % (time.ctime(), bookbagofholding.UPDATE_MSG))
                    logger.info(bookbagofholding.UPDATE_MSG)
                    ...
                    myDB.action('CREATE TABLE users (UserID TEXT UNIQUE, UserName TEXT UNIQUE, Password TEXT, ' +
                                ...
                                'Role TEXT DEFAULT "guest", ApiKey TEXT, LastLogin TEXT, ' +
                                'CreatedAt TEXT, PasswordChangedAt TEXT, PasswordAlgorithm TEXT DEFAULT "md5", ' +
                                'PasswordResetToken TEXT, PasswordResetExpiry TEXT)')
                    myDB.action('CREATE TABLE sync (UserID TEXT REFERENCES users (UserID) ON DELETE CASCADE, ' +
                                'Label TEXT, Date TEXT, SyncList TEXT)')
                    myDB.action('CREATE TABLE sessions (SessionID TEXT PRIMARY KEY, ' +
                                'UserID TEXT REFERENCES users (UserID) ON DELETE CASCADE, ' +
                                'CreatedAt TEXT, ExpiresAt TEXT, IPAddress TEXT, UserAgent TEXT)')
                    ...
                    myDB.action('CREATE INDEX sessions_user_index ON sessions (UserID)')
                    myDB.action('CREATE INDEX sessions_expires_index ON sessions (ExpiresAt)')

                index = db_version + 1
                while 'db_v%s' % index in globals():
                    upgrade_function = getattr(bookbagofholding.dbupgrade, 'db_v%s' % index)
                ...
                myDB.action('vacuum')
                bookbagofholding.UPDATE_MSG = 'Database updated to version %s' % db_current_version
                logger.info(bookbagofholding.UPDATE_MSG)
                upgradelog.write("%s: %s\n" % (time.ctime(), bookbagofholding.UPDATE_MSG))
...
def db_v2(myDB, upgradelog):
    try:
        ...
        if results:
            bookbagofholding.UPDATE_MSG = 'Removing units from wanted table'
            logger.debug(bookbagofholding.UPDATE_MSG)
            upgradelog.write("%s v2: %s\n" % (time.ctime(), bookbagofholding.UPDATE_MSG))
            tot = len(results)
            cnt = 0
            for units in results:
                cnt += 1
                bookbagofholding.UPDATE_MSG = 'Removing units from wanted table: %s of %s' % (cnt, tot)
...
def db_v3(myDB, upgradelog):
    if has_column(myDB, "books", "SeriesOrder"):
        bookbagofholding.UPDATE_MSG = 'Removing SeriesOrder from books table'
    ...
    if not has_column(myDB, "pastissues", "BookID"):
        bookbagofholding.UPDATE_MSG = 'Moving magazine past issues into new table'
        upgradelog.write("%s v3: %s\n" % (time.ctime(), bookbagofholding.UPDATE_MSG))
        logger.debug(bookbagofholding.UPDATE_MSG)
...
def db_v4(myDB, upgradelog):
    if not has_column(myDB, "stats", "duplicates"):
        bookbagofholding.UPDATE_MSG = 'Updating stats table to hold duplicates'
...
def db_v5(myDB, upgradelog):
    ...
    if issues:
        bookbagofholding.UPDATE_MSG = 'Updating issues table to hold 4 digit issue numbers'
        ...
        for issue in issues:
            cnt += 1
            bookbagofholding.UPDATE_MSG = 'Updating issues table 4 digits: %s of %s' % (cnt, tot)
    ...
    if mags:
        bookbagofholding.UPDATE_MSG = 'Updating magazines table to 4 digits'
        upgradelog.write("%s v5: %s\n" % (time.ctime(), bookbagofholding.UPDATE_MSG))
        logger.debug(bookbagofholding.UPDATE_MSG)
        tot = len(mags)
        cnt = 0
        for mag in mags:
            cnt += 1
            bookbagofholding.UPDATE_MSG = 'Updating magazines table to 4 digits: %s of %s' % (cnt, tot)
            title = mag['Title']
...
def db_v6(myDB, upgradelog):
    if not has_column(myDB, "books", "Manual"):
        bookbagofholding.UPDATE_MSG = 'Updating books table to hold Manual setting'
...
def db_v7(myDB, upgradelog):
    if not has_column(myDB, "wanted", "Source"):
        bookbagofholding.UPDATE_MSG = 'Updating wanted table to hold Source and DownloadID'
...
def db_v8(myDB, upgradelog):
    src = os.path.join(bookbagofholding.PROG_DIR, 'data/images/cache/')
    dst = bookbagofholding.CACHEDIR
    images = myDB.select(
        'SELECT AuthorID, AuthorImg FROM authors WHERE AuthorImg LIKE "images/cache/%"')
    if images:
        bookbagofholding.UPDATE_MSG = 'Moving author images to new location'
        logger.debug(bookbagofholding.UPDATE_MSG)
        ...
        for image in images:
            cnt += 1
            bookbagofholding.UPDATE_MSG = "Moving author images to new location: %s of %s" % (cnt, tot)
            img = image['AuthorImg']
            img = img[7:]
            myDB.action(
            ...
            if os.path.isfile(srcfile):
                try:
                    ...
                    logger.error(msg)
                    upgradelog.write("%s v8: %s\n" % (time.ctime(), msg))
        upgradelog.write("%s v8: %s" % (time.ctime(), bookbagofholding.UPDATE_MSG))
    ...
    if images:
        bookbagofholding.UPDATE_MSG = 'Moving book images to new location'
        logger.debug(bookbagofholding.UPDATE_MSG)
        upgradelog.write("%s v8: %s\n" % (time.ctime(), bookbagofholding.UPDATE_MSG))
        tot = len(images)
        cnt = 0
        for image in images:
            cnt += 1
            bookbagofholding.UPDATE_MSG = "Moving book images to new location: %s of %s" % (cnt, tot)
            img = image['BookImg']
            img = img[7:]
...
def db_v9(myDB, upgradelog):
    if not has_column(myDB, "magazines", "Reject"):
        ...
        bookbagofholding.UPDATE_MSG = 'Updating magazines table'
...
def db_v11(myDB, upgradelog):
    ...
    if not has_column(myDB, "authors", "LastBookImg"):
        bookbagofholding.UPDATE_MSG = 'Updating author table to hold last book image'
        ...
        if books:
            for book in books:
                bookbagofholding.UPDATE_MSG = 'Updating last book image for %s' % book['AuthorName']
                if book['LastBook']:
                    match = myDB.match(
...
def db_v12(myDB, upgradelog):
    ...
    if not has_column(myDB, "Magazines", "LatestCover"):
        bookbagofholding.UPDATE_MSG = 'Updating magazine table to hold last issue image'
        ...
        if mags:
            for mag in mags:
                bookbagofholding.UPDATE_MSG = 'Updating last issue image for %s' % mag['Title']
                match = myDB.match(
...
def db_v13(myDB, upgradelog):
    if not has_column(myDB, "authors", "Manual"):
        bookbagofholding.UPDATE_MSG = 'Updating authors table to hold Manual setting'
...
def db_v15(myDB, upgradelog):
    ...
    if has_column(myDB, "books", "SeriesNum"):
        bookbagofholding.UPDATE_MSG = 'Removing seriesnum from books table'
        ...
        bookbagofholding.UPDATE_MSG = 'Reorganisation of books table complete'
    upgradelog.write("%s v15: complete\n" % time.ctime())


def db_v16(myDB, upgradelog):
    if has_column(myDB, "books", "AuthorLink"):
        bookbagofholding.UPDATE_MSG = 'Removing series, authorlink and authorname from books table'
...
def db_v17(myDB, upgradelog):
    if has_column(myDB, "series", "AuthorID"):
        bookbagofholding.UPDATE_MSG = 'Creating seriesauthors table'
        ...
        for item in series:
            cnt += 1
            bookbagofholding.UPDATE_MSG = "Updating seriesauthors: %s of %s" % (cnt, tot)
            if item['AuthorID']:
        ...
        bookbagofholding.UPDATE_MSG = 'Reorganisation of series table complete'
        upgradelog.write("%s v17: %s\n" % (time.ctime(), bookbagofholding.UPDATE_MSG))
...
def db_v18(myDB, upgradelog):
    ...
    if not data:
        bookbagofholding.UPDATE_MSG = 'Adding unique constraint to seriesauthors table'
        ...
        bookbagofholding.UPDATE_MSG = 'Reorganisation of seriesauthors complete'
        upgradelog.write("%s v18: %s\n" % (time.ctime(), bookbagofholding.UPDATE_MSG))
...
def db_v19(myDB, upgradelog):
    if not has_column(myDB, "books", "SeriesDisplay"):
        bookbagofholding.UPDATE_MSG = 'Adding series display to book table'
        ...
        if books:
            ...
            for book in books:
                cnt += 1
                bookbagofholding.UPDATE_MSG = "Updating series display: %s of %s" % (cnt, tot)
        ...
        bookbagofholding.UPDATE_MSG = 'Reorganisation of series display complete'
        upgradelog.write("%s v19: %s\n" % (time.ctime(), bookbagofholding.UPDATE_MSG))
...
def db_v20(myDB, upgradelog):
    if not has_column(myDB, "books", "BookLibrary"):
        bookbagofholding.UPDATE_MSG = 'Adding BookLibrary to book table'
        upgradelog.write("%s v20: %s\n" % (time.ctime(), bookbagofholding.UPDATE_MSG))
        myDB.action('ALTER TABLE books ADD COLUMN BookLibrary TEXT')

        bookbagofholding.UPDATE_MSG = 'Updating BookLibrary dates'
        upgradelog.write("%s v20: %s\n" % (time.ctime(), bookbagofholding.UPDATE_MSG))
        ...
        if books:
            ...
            for book in books:
                cnt += 1
                bookbagofholding.UPDATE_MSG = "Updating BookLibrary date: %s of %s" % (cnt, tot)
            ...
            bookbagofholding.UPDATE_MSG = 'Adding BookLibrary date complete, %s/%s books' % (mod, cnt)
            upgradelog.write("%s v20: %s\n" % (time.ctime(), bookbagofholding.UPDATE_MSG))
...
def db_v21(myDB, upgradelog):
    if not has_column(myDB, "books", "AudioLibrary"):
        bookbagofholding.UPDATE_MSG = 'Adding AudioBook support to book table'
        ...
        bookbagofholding.UPDATE_MSG = 'Creating downloads table'
        upgradelog.write("%s v21: %s\n" % (time.ctime(), bookbagofholding.UPDATE_MSG))
...
def db_v22(myDB, upgradelog):
    if not has_column(myDB, "authors", "GRfollow"):
        bookbagofholding.UPDATE_MSG = 'Adding Goodreads Follow support to author table'
...
def db_v23(myDB, upgradelog):
    if not has_column(myDB, "users", "Perms"):
        bookbagofholding.UPDATE_MSG = 'Adding Users table'
        ...
        user = bookbagofholding.CONFIG['HTTP_USER']
        pwd = bookbagofholding.CONFIG['HTTP_PASS']
        email = bookbagofholding.CONFIG['ADMIN_EMAIL']
        name = 'admin'
        if not user or not pwd:
            user = 'admin'
            pwd = 'admin'
        myDB.action(cmd, (pwd_generator(), user, name, md5_utf8(pwd), email,
                          bookbagofholding.perm_admin))
...
def db_v24(myDB, upgradelog):
    if not has_column(myDB, "users", "HaveRead"):
        bookbagofholding.UPDATE_MSG = 'Adding read lists to Users table'
...
def db_v26(myDB, upgradelog):
    if not has_column(myDB, "sync", "UserID"):
        bookbagofholding.UPDATE_MSG = 'Adding sync table'
...
def db_v28(myDB, upgradelog):
    if not has_column(myDB, "users", "CalibreRead"):
        bookbagofholding.UPDATE_MSG = 'Adding Calibre column names to Users table'
...
def db_v29(myDB, upgradelog):
    if not has_column(myDB, "books", "WorkID"):
        bookbagofholding.UPDATE_MSG = 'Adding WorkID to member and books tables'
    ...
    if not has_column(myDB, "member", "WorkID"):
        ...
        myDB.action('DROP TABLE temp_table')
    if bookbagofholding.CONFIG['BOOK_API'] == 'GoodReads':
        ...
        if tot:
            ...
            for author in authors:
                ...
                if not expected_books:
                    expected_books = '0'
                bookbagofholding.UPDATE_MSG = "Updating %s (%s books): %s" % (author['AuthorName'], expected_books,
        ...
        if tot:
            ...
            for member in members:
                cnt += 1
                bookbagofholding.UPDATE_MSG = "Updating series members %s of %s" % (cnt, tot)
...
def db_v30(myDB, upgradelog):
    if not has_column(myDB, "users", "BookType"):
        bookbagofholding.UPDATE_MSG = 'Adding BookType to Users table'
...
def db_v31(myDB, upgradelog):
    if not has_column(myDB, "magazines", "DateType"):
        bookbagofholding.UPDATE_MSG = 'Adding DateType to Magazines table'
...
def db_v32(myDB, upgradelog):
    if not has_column(myDB, "series", "Have"):
        bookbagofholding.UPDATE_MSG = 'Adding counters to Series table'
    ...
    if tot:
        ...
        for entry in series:
            cnt += 1
            bookbagofholding.UPDATE_MSG = "Updating series counters %s of %s" % (cnt, tot)
...
def db_v33(myDB, upgradelog):
    if not has_column(myDB, "wanted", "DLResult"):
        bookbagofholding.UPDATE_MSG = 'Adding DLResult to wanted table'
...
def db_v34(myDB, upgradelog):
    if not has_column(myDB, "books", "ScanResult"):
        bookbagofholding.UPDATE_MSG = 'Adding ScanResult to books table'
...
def db_v35(myDB, upgradelog):
    if not has_column(myDB, "books", "OriginalPubDate"):
        bookbagofholding.UPDATE_MSG = 'Adding OriginalPubDate to books table'
...
def db_v36(myDB, upgradelog):
    if not has_column(myDB, "failedsearch", "Time"):
        bookbagofholding.UPDATE_MSG = 'Creating failed search table'
...
def db_v37(myDB, upgradelog):
    bookbagofholding.UPDATE_MSG = 'Adding delete cascade constraint to existing tables'
...
def db_v38(myDB, upgradelog):
    bookbagofholding.UPDATE_MSG = 'Changing counters in series table to integers'
...
def db_v39(myDB, upgradelog):
    if not has_column(myDB, "authors", "LastBookID"):
        bookbagofholding.UPDATE_MSG = 'Updating author table to hold LastBookID setting'
    ...
    for author in authors:
        cnt += 1
        bookbagofholding.UPDATE_MSG = 'Updating last book for %s: %s' % (author['AuthorName'],
...
def db_v40(myDB, upgradelog):
    if not has_column(myDB, "magazines", "CoverPage"):
        bookbagofholding.UPDATE_MSG = 'Updating magazines to hold CoverPage setting'
...
def db_v41(myDB, upgradelog):
    if not has_column(myDB, "books", "Requester"):
        bookbagofholding.UPDATE_MSG = 'Updating books to hold requested wishlist name'
...
def db_v42(myDB, upgradelog):
    if not has_column(myDB, "users", "SendTo"):
        bookbagofholding.UPDATE_MSG = 'Adding SendTo to Users table'
...
def db_v43(myDB, upgradelog):
    bookbagofholding.UPDATE_MSG = 'Removing foreign key from wanted table'
...
def db_v45(myDB, upgradelog):
    ...
    bookbagofholding.UPDATE_MSG = 'Removing series data (series functionality removed)'
...
def db_v46(myDB, upgradelog):
    ...
    bookbagofholding.UPDATE_MSG = 'Creating blacklist table for processed downloads'
...
def db_v47(myDB, upgradelog):
    ...
    bookbagofholding.UPDATE_MSG = 'Creating unmatchedfiles table for import tracking'
...
def db_v48(myDB, upgradelog):
    # Add new columns to users table for RBAC and session management
    bookbagofholding.UPDATE_MSG = 'Adding Role and session columns to Users table'
    upgradelog.write("%s v48: %s\n" % (time.ctime(), bookbagofholding.UPDATE_MSG))

    if not has_column(myDB, "users", "Role"):
        myDB.action('ALTER TABLE users ADD COLUMN Role TEXT DEFAULT "guest"')
    if not has_column(myDB, "users", "ApiKey"):
        myDB.action('ALTER TABLE users ADD COLUMN ApiKey TEXT')
    if not has_column(myDB, "users", "LastLogin"):
        myDB.action('ALTER TABLE users ADD COLUMN LastLogin TEXT')
    if not has_column(myDB, "users", "CreatedAt"):
        myDB.action('ALTER TABLE users ADD COLUMN CreatedAt TEXT')
    if not has_column(myDB, "users", "PasswordChangedAt"):
        myDB.action('ALTER TABLE users ADD COLUMN PasswordChangedAt TEXT')
    if not has_column(myDB, "users", "PasswordAlgorithm"):
        myDB.action('ALTER TABLE users ADD COLUMN PasswordAlgorithm TEXT DEFAULT "md5"')
...
def db_v49(myDB, upgradelog):
    ...
    bookbagofholding.UPDATE_MSG = 'Creating sessions table for session management'
    upgradelog.write("%s v49: %s\n" % (time.ctime(), bookbagofholding.UPDATE_MSG))

    myDB.action('CREATE TABLE IF NOT EXISTS sessions ('
                ...
                'IPAddress TEXT, '
                'UserAgent TEXT)')
    myDB.action('CREATE INDEX IF NOT EXISTS sessions_user_index ON sessions (UserID)')
    myDB.action('CREATE INDEX IF NOT EXISTS sessions_expires_index ON sessions (ExpiresAt)')

    upgradelog.write("%s v49: complete\n" % time.ctime())


def db_v50(myDB, upgradelog):
    # Migrate existing users to set Role based on Perms, and set default values
    bookbagofholding.UPDATE_MSG = 'Migrating existing users to role-based system'
    upgradelog.write("%s v50: %s\n" % (time.ctime(), bookbagofholding.UPDATE_MSG))

    now = datetime.datetime.utcnow().strftime("%Y-%m-%d %H:%M:%S")

    # Set Role based on existing Perms value
    # Admin (65535) -> admin
    # Friend (5856) -> friend
    # Guest (4320) -> guest
    # Others -> guest
    users = myDB.select('SELECT UserID, Perms, Role FROM users')
    for user in users:
        if not user['Role']:  # Only update if Role not already set
            perms = user['Perms'] or 0
            if perms >= 65535:
                role = 'admin'
            elif perms >= 5856:
                role = 'friend'
                ...
                role = 'guest'
            myDB.action('UPDATE users SET Role=?, CreatedAt=?, PasswordAlgorithm=? WHERE UserID=?',
                        (role, now, 'md5', user['UserID']))

    upgradelog.write("%s v50: complete\n" % time.ctime())


def db_v51(myDB, upgradelog):
    # Add password reset token columns
    bookbagofholding.UPDATE_MSG = 'Adding password reset token columns to Users table'
    upgradelog.write("%s v51: %s\n" % (time.ctime(), bookbagofholding.UPDATE_MSG))

    if not has_column(myDB, "users", "PasswordResetToken"):
        myDB.action('ALTER TABLE users ADD COLUMN PasswordResetToken TEXT')
    if not has_column(myDB, "users", "PasswordResetExpiry"):
