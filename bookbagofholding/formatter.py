...
def multibook(foldername, recurse=False):
    ...
    filetypes = getList(bookbagofholding.CONFIG['EBOOK_TYPE'])
...
def next_run(when_run):
    ...
    try:
        ...
        bookbagofholding.logger.error("Error getting next run for [%s] %s" % (when_run, str(e)))
        diff = 0
        td = ''
...
def dateFormat(datestr, formatstr):
    ...
    # datestr are stored in bookbagofholding as YYYY-MM-DD or YYYY-MM-DD HH:MM:SS or nnnn for issue number
    ...
    try:
        return formatstr.replace(
            ...
            '$B', bookbagofholding.MONTHNAMES[int(datestr[5:7])][0].title()).replace(
            '$b', bookbagofholding.MONTHNAMES[int(datestr[5:7])][1].title())
    except Exception:
        return datestr
...
def month2num(month):
    ...
    month = month.lower()
    for f in range(1, 13):
        if month in bookbagofholding.MONTHNAMES[f]:
            return f
...
def makeUnicode(txt):
    ...
    if not txt:
        ...
        return txt
    for encoding in [bookbagofholding.SYS_ENCODING, 'latin-1', 'utf-8']:
        try:
            txt = txt.decode(encoding)
            return txt
        except UnicodeError:
            pass
    bookbagofholding.logger.debug("Unable to decode name [%s]" % repr(txt))
    return txt
...
def makeBytestr(txt):
    ...
    for encoding in [bookbagofholding.SYS_ENCODING, 'latin-1', 'utf-8']:
        try:
            ...
            pass
    bookbagofholding.logger.debug("Unable to encode name [%s]" % repr(txt))
    return txt
...
def formatAuthorName(author):
    ...
    if "," in author:
        postfix = getList(bookbagofholding.CONFIG['NAME_POSTFIX'])
        words = author.split(',')
        if len(words) == 2:
            ...
            if author != forename + ' ' + surname:
                bookbagofholding.logger.debug('Formatted authorname [%s] to [%s %s]' % (author, forename, surname))
    ...
    if len(author) > 2 and author[1] in '. ':
        ...
        if author != forename + ' ' + surname:
            bookbagofholding.logger.debug('Stripped authorname [%s] to [%s %s]' % (author, forename, surname))
            author = forename + ' ' + surname
...
def surnameFirst(authorname):
    ...
    if res.strip('.').lower in getList(bookbagofholding.CONFIG['NAME_POSTFIX']):
        res = words.pop() + ' ' + res
...
def cleanName(name, extras=None):
    ...
    try:
        ...
        cleanedName = unicodedata.normalize('NFKD', name.decode(bookbagofholding.SYS_ENCODING)).encode('ASCII', 'ignore')
...
def unaccented_str(str_or_unicode):
    ...
    return stripped.decode(bookbagofholding.SYS_ENCODING)
...
def dispName(provider):
    ...
    for item in bookbagofholding.NEWZNAB_PROV:
        if item['HOST'].strip('/') == provider:
            ...
            break
    if not provname:
        for item in bookbagofholding.TORZNAB_PROV:
            if item['HOST'].strip('/') == provider:
                provname = item['DISPNAME']
                break
    if not provname:
        for item in bookbagofholding.RSS_PROV:
            if item['HOST'].strip('/') == provider:
                provname = item['DISPNAME']
                break
    if not provname:
