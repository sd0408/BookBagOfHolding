...
from bookbagofholding.formatter import plural, next_run, is_valid_booktype, datecompare, check_int, \
...
def getUserAgent():
    ...
    if bookbagofholding.CONFIG['USER_AGENT']:
        return bookbagofholding.CONFIG['USER_AGENT']
    else:
        return 'Bookbag of Holding' + ' (' + platform.system() + ' ' + platform.release() + ')'
...
def mymakedirs(dest_path):
    ...
    for entry in to_make:
        if bookbagofholding.LOGLEVEL & bookbagofholding.log_fileperms:
...
def proxyList():
    proxies = None
    if bookbagofholding.CONFIG['PROXY_HOST']:
        proxies = {}
        for item in getList(bookbagofholding.CONFIG['PROXY_TYPE']):
            if item in ['http', 'https']:
                proxies.update({item: bookbagofholding.CONFIG['PROXY_HOST']})
    return proxies
...
<html>
    ...
    <h1>Bookbag of Holding<br><br></h1>
...
def setperm(file_or_dir):
    ...
    if os.path.isdir(file_or_dir):
        perm = octal(bookbagofholding.CONFIG['DIR_PERM'], 0o755)
    elif os.path.isfile(file_or_dir):
        perm = octal(bookbagofholding.CONFIG['FILE_PERM'], 0o644)
    else:
...
def bts_file(search_dir=None):
    if 'bts' not in getList(bookbagofholding.CONFIG['SKIPPED_EXT']):
        return ''
...
def is_overdue():
    ...
    name = ''
    days = 0
    maxage = check_int(bookbagofholding.CONFIG['CACHE_AGE'], 0)
    if maxage:
...
# Job ID constants for APScheduler v3
JOB_ID_POSTPROCESSOR = 'job_postprocessor'
JOB_ID_SEARCH_BOOK = 'job_search_book'
JOB_ID_SEARCH_RSS = 'job_search_rss_book'
JOB_ID_SEARCH_WISHLIST = 'job_search_wishlist'
JOB_ID_AUTHOR_UPDATE = 'job_author_update'
JOB_ID_SESSION_CLEANUP = 'job_session_cleanup'

# Map target names to job IDs
JOB_TARGET_MAP = {
    'processDir': JOB_ID_POSTPROCESSOR,
    'PostProcessor': JOB_ID_POSTPROCESSOR,
    'search_book': JOB_ID_SEARCH_BOOK,
    'search_rss_book': JOB_ID_SEARCH_RSS,
    'search_wishlist': JOB_ID_SEARCH_WISHLIST,
    'authorUpdate': JOB_ID_AUTHOR_UPDATE,
    'session_cleanup': JOB_ID_SESSION_CLEANUP,
}
...
def scheduleJob(action='Start', target=None):
    ...
    job_id = JOB_TARGET_MAP.get(target)
    if not job_id:
        logger.debug("Unknown job target: %s" % target)
        return

    if action in ['Stop', 'Restart']:
        existing_job = bookbagofholding.SCHED.get_job(job_id)
        if existing_job:
            bookbagofholding.SCHED.remove_job(job_id)
            logger.debug("Stop %s job" % target)

    if action in ['Start', 'Restart']:
        existing_job = bookbagofholding.SCHED.get_job(job_id)
        if existing_job and action == 'Start':
            logger.debug("%s %s job, already scheduled" % (action, target))
            return  # return if already running, if not, start a new one

        if 'processDir' in target and check_int(bookbagofholding.CONFIG['SCAN_INTERVAL'], 0):
            minutes = check_int(bookbagofholding.CONFIG['SCAN_INTERVAL'], 0)
            bookbagofholding.SCHED.add_job(
                bookbagofholding.postprocess.cron_processDir, 'interval',
                minutes=minutes, id=job_id, replace_existing=True
            )
            logger.debug("%s %s job in %s minute%s" % (action, target, minutes, plural(minutes)))
        elif 'search_book' in target and check_int(bookbagofholding.CONFIG['SEARCH_BOOKINTERVAL'], 0):
            minutes = check_int(bookbagofholding.CONFIG['SEARCH_BOOKINTERVAL'], 0)
            if bookbagofholding.USE_NZB() or bookbagofholding.USE_TOR() or bookbagofholding.USE_DIRECT():
                bookbagofholding.SCHED.add_job(
                    bookbagofholding.searchbook.cron_search_book, 'interval',
                    minutes=minutes, id=job_id, replace_existing=True
                )
                logger.debug("%s %s job in %s minute%s" % (action, target, minutes, plural(minutes)))
        elif 'search_rss_book' in target and check_int(bookbagofholding.CONFIG['SEARCHRSS_INTERVAL'], 0):
            if bookbagofholding.USE_RSS():
                minutes = check_int(bookbagofholding.CONFIG['SEARCHRSS_INTERVAL'], 0)
                bookbagofholding.SCHED.add_job(
                    bookbagofholding.searchrss.cron_search_rss_book, 'interval',
                    minutes=minutes, id=job_id, replace_existing=True
                )
                logger.debug("%s %s job in %s minute%s" % (action, target, minutes, plural(minutes)))
        elif 'search_wishlist' in target and check_int(bookbagofholding.CONFIG['WISHLIST_INTERVAL'], 0):
            if bookbagofholding.USE_WISHLIST():
                hours = check_int(bookbagofholding.CONFIG['WISHLIST_INTERVAL'], 0)
                bookbagofholding.SCHED.add_job(
                    bookbagofholding.searchrss.cron_search_wishlist, 'interval',
                    hours=hours, id=job_id, replace_existing=True
                )
                logger.debug("%s %s job in %s hour%s" % (action, target, hours, plural(hours)))
        elif 'authorUpdate' in target and check_int(bookbagofholding.CONFIG['CACHE_AGE'], 0):
        ...
        elif 'session_cleanup' in target:
            # Session cleanup runs every hour
            bookbagofholding.SCHED.add_job(
                cron_session_cleanup, 'interval',
                hours=1, id=job_id, replace_existing=True
            )
            logger.debug("%s session_cleanup job (hourly)" % action)


def cron_session_cleanup():
    """Scheduled job to clean up expired sessions."""
    threadname = threading.currentThread().name
    if "Thread-" in threadname:
        threading.currentThread().name = "SESSIONCLEANUP"
    try:
        from bookbagofholding.web.auth import cleanup_expired_sessions
        deleted = cleanup_expired_sessions()
        if deleted > 0:
            logger.debug("Session cleanup: removed %s expired session%s" % (deleted, plural(deleted)))
    except Exception:
        logger.error('Unhandled exception in session cleanup: %s' % traceback.format_exc())
...
def authorUpdate(restart=True):
    ...
    try:
        ...
        if author and check_int(bookbagofholding.CONFIG['CACHE_AGE'], 0):
            dtnow = datetime.datetime.now()
            diff = datecompare(dtnow.strftime("%Y-%m-%d"), author['DateAdded'])
            msg = 'Oldest author info (%s) is %s day%s old' % (author['AuthorName'], diff, plural(diff))
            if diff > check_int(bookbagofholding.CONFIG['CACHE_AGE'], 0):
                msg = 'Starting update for %s' % author['AuthorName']
                logger.info(msg)
                bookbagofholding.importer.addAuthorToDB(refresh=True, authorid=author['AuthorID'])
...
def restartJobs(start='Restart'):
    ...
    scheduleJob(start, 'session_cleanup')
    ...
    if start == 'Start':
        try:
            ...
            deleted = cleanup_expired_sessions()
            if deleted:
                logger.info("Startup: cleaned up %s expired session%s" % (deleted, plural(deleted)))
        except Exception as e:
            logger.debug("Session cleanup on startup: %s" % str(e))


def ensureRunning(jobname):
    job_id = JOB_TARGET_MAP.get(jobname)
    if job_id:
        existing_job = bookbagofholding.SCHED.get_job(job_id)
        if not existing_job:
            scheduleJob('Start', jobname)
    else:
        # Fallback for unknown job names
        scheduleJob('Start', jobname)
...
def showStats():
    gb_status = "Active"
    for entry in bookbagofholding.PROVIDER_BLOCKLIST:
        if entry["name"] == 'googleapis':
            if int(time.time()) < int(entry['resume']):
                gb_status = "Blocked"
            break

    result = ["Cache %i hit%s, %i miss, " % (check_int(bookbagofholding.CACHE_HIT, 0),
                                             plural(check_int(bookbagofholding.CACHE_HIT, 0)),
                                             check_int(bookbagofholding.CACHE_MISS, 0)),
              "Sleep %.3f librarything" % bookbagofholding.LT_SLEEP,
              "GoogleBooks API %i calls, %s" % (bookbagofholding.GB_CALLS, gb_status)]
...
def showJobs():
    ...
    # Map job IDs to display names
    job_display_names = {
        JOB_ID_POSTPROCESSOR: "PostProcessor",
        JOB_ID_SEARCH_BOOK: "Book search",
        JOB_ID_SEARCH_RSS: "RSS book search",
        JOB_ID_SEARCH_WISHLIST: "Wishlist search",
        JOB_ID_AUTHOR_UPDATE: "Update authors",
    }

    for job in bookbagofholding.SCHED.get_jobs():
        jobname = job_display_names.get(job.id, job.id)

        if job.next_run_time:
            # Format the next run time
            jobtime = next_run(job.next_run_time.strftime("%Y-%m-%d %H:%M:%S"))
...
def clearLog():
    ...
    logger.bookbagofholding_log.stopLogger()
    for f in glob.glob(bookbagofholding.CONFIG['LOGDIR'] + "/*.log*"):
        try:
            os.remove(f)
        except OSError as e:
            error = e.strerror
    ...
    logger.bookbagofholding_log.initLogger(loglevel=bookbagofholding.LOGLEVEL)

    if error:
        return 'Failed to clear logfiles: %s' % error
    else:
        return "Log cleared, level set to [%s]- Log Directory is [%s]" % (
            bookbagofholding.LOGLEVEL, bookbagofholding.CONFIG['LOGDIR'])
...
# noinspection PyUnresolvedReferences
def logHeader():
    popen_list = [sys.executable, bookbagofholding.FULL_PATH]
    popen_list += bookbagofholding.ARGS
    header = "Startup cmd: %s\n" % str(popen_list)
    header += 'Interface: modern\n'
    header += 'Loglevel: %s\n' % bookbagofholding.LOGLEVEL
    header += 'Sys_Encoding: %s\n' % bookbagofholding.SYS_ENCODING
    for item in bookbagofholding.CONFIG_GIT:
        if item == 'GIT_UPDATED':
            timestamp = check_int(bookbagofholding.CONFIG[item], 0)
            header += '%s: %s\n' % (item.lower(), time.ctime(timestamp))
        else:
            header += '%s: %s\n' % (item.lower(), bookbagofholding.CONFIG[item])
    ...
    if not bookbagofholding.GROUP_CONCAT:
...
def saveLog():
    if not os.path.exists(bookbagofholding.CONFIG['LOGDIR']):
        return 'LOGDIR does not exist'

    basename = os.path.join(bookbagofholding.CONFIG['LOGDIR'], 'bookbagofholding.log')
    outfile = os.path.join(bookbagofholding.CONFIG['LOGDIR'], 'debug')
    ...
    with open(outfile + '.tmp', 'w') as out:
        ...
        if os.path.exists(bookbagofholding.CONFIGFILE):
            out.write('---END-CONFIG---------------------------------\n')
            for line in reverse_readline(bookbagofholding.CONFIGFILE):
                for item in redactlist:
                    item = item.replace('->', '=')
                    startpos = line.find(item)
                    if startpos >= 0:
...
def cleanCache():
    ...
    myDB = database.DBConnection()
    result = []
    expiry = check_int(bookbagofholding.CONFIG['CACHE_AGE'], 0)
    cache = os.path.join(bookbagofholding.CACHEDIR, "JSONCache")
    cleaned = 0
    kept = 0
    ...
    cache = os.path.join(bookbagofholding.CACHEDIR, "XMLCache")
    cleaned = 0
    kept = 0
    if expiry and os.path.isdir(cache):
        for cached_file in os.listdir(makeBytestr(cache)):
    ...
    cache = os.path.join(bookbagofholding.CACHEDIR, "WorkCache")
    cleaned = 0
    kept = 0
    ...
    cache = os.path.join(bookbagofholding.CACHEDIR, "SeriesCache")
    cleaned = 0
    kept = 0
    if os.path.isdir(cache):
        for cached_file in os.listdir(makeBytestr(cache)):
    ...
    cache = bookbagofholding.CACHEDIR
    cleaned = 0
    kept = 0
    ...
    cachedir = os.path.join(bookbagofholding.CACHEDIR, 'book')
    cleaned = 0
    kept = 0
    for item in images:
        keep = True
    ...
    cachedir = os.path.join(bookbagofholding.CACHEDIR, 'author')
    cleaned = 0
    kept = 0
    for item in images:
        keep = True
