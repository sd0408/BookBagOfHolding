...
def processResultList(resultlist, book, searchtype, source):
    ...
    if match:
        ...
        if score < int(bookbagofholding.CONFIG['MATCH_RATIO']):
            return 0
...
def findBestResult(resultlist, book, searchtype, source):
    ...
    try:
        ...
        if book['library'] == 'AudioBook':
            reject_list = getList(bookbagofholding.CONFIG['REJECT_AUDIO'], ',')
            maxsize = check_int(bookbagofholding.CONFIG['REJECT_MAXAUDIO'], 0)
            minsize = check_int(bookbagofholding.CONFIG['REJECT_MINAUDIO'], 0)
            auxinfo = 'AudioBook'

        else:  # elif book['library'] == 'eBook':
            reject_list = getList(bookbagofholding.CONFIG['REJECT_WORDS'], ',')
            maxsize = check_int(bookbagofholding.CONFIG['REJECT_MAXSIZE'], 0)
            minsize = check_int(bookbagofholding.CONFIG['REJECT_MINSIZE'], 0)
            auxinfo = 'eBook'
        ...
        for res in resultlist:
            ...
            if not rejected:
                ...
                if newValueDict['AuxInfo'] == 'eBook':
                    words = [x for x in words if x not in getList(bookbagofholding.CONFIG['EBOOK_TYPE'])]
                    typelist = getList(bookbagofholding.CONFIG['EBOOK_TYPE'])
                elif newValueDict['AuxInfo'] == 'AudioBook':
                    words = [x for x in words if x not in getList(bookbagofholding.CONFIG['AUDIOBOOK_TYPE'])]
                    typelist = getList(bookbagofholding.CONFIG['AUDIOBOOK_TYPE'])
                score -= len(words)
