...
from bookbagofholding import database, logger, utorrent, transmission, qbittorrent, \
    deluge, rtorrent, synology, sabnzbd, nzbget
from bookbagofholding.database import add_to_blacklist
from bookbagofholding.bookrename import nameVars, audioProcess, stripspaces, id3read
from bookbagofholding.cache import cache_img
from bookbagofholding.calibre import calibredb
from bookbagofholding.common import scheduleJob, book_file, opf_file, setperm, bts_file, jpg_file, \
    safe_copy, safe_move, mymakedirs, runScript, get_files_by_type
from bookbagofholding.formatter import unaccented_str, unaccented, plural, now, today, is_valid_booktype, \
    replace_all, getList, surnameFirst, makeUnicode, makeBytestr, check_int, is_valid_type, multibook
from bookbagofholding.importer import addAuthorToDB, addAuthorNameToDB, update_totals
from bookbagofholding.librarysync import get_book_info, find_book_in_db, LibraryScan
...
def fail_type_mismatch(book, book_type, pp_path, files_found):
    ...
    if book_type == 'eBook':
        found_type = 'audiobook' if files_found['audiobook'] else 'other'
        found_files = files_found['audiobook'] if files_found['audiobook'] else files_found['other']
    else:
        found_type = 'ebook' if files_found['ebook'] else 'other'
        found_files = files_found['ebook'] if files_found['ebook'] else files_found['other']

    file_sample = found_files[:3] if len(found_files) > 3 else found_files
    file_desc = ', '.join(file_sample)
    if len(found_files) > 3:
        file_desc += ' (and %d more)' % (len(found_files) - 3)

    error_msg = "Type mismatch: expected %s but found %s files (%s)" % (
        book_type, found_type, file_desc)
    ...
    # Reset book status to Wanted so it can retry with another provider
...
def processAlternate(source_dir=None, library='eBook'):
    ...
    try:
        ...
        if not os.path.isdir(source_dir):
            ...
            return False
        if source_dir == bookbagofholding.DIRECTORY('eBook'):
        ...
        if library == 'eBook':
            ...
            # depending on bookbagofholding.CONFIG['DESTINATION_COPY'] setting
        ...
        if ourfile.startswith(fname) or is_valid_booktype(ourfile, booktype="audiobook"):
            if is_valid_type(ourfile):
                try:
                    ...
                    if bookbagofholding.CONFIG['DESTINATION_COPY'] or move == 'copy':
...
def unpack_archive(archivename, download_dir, title):
    ...
    if zipfile.is_zipfile(archivename):
        if bookbagofholding.LOGLEVEL & bookbagofholding.log_postprocess:
...
def processDir(reset=False, startdir=None, ignoreclient=False):
    ...
    try:
        ppcount = 0
        myDB = database.DBConnection()
        skipped_extensions = getList(bookbagofholding.CONFIG['SKIPPED_EXT'])
        if startdir:
            templist = [startdir]
        else:
            templist = getList(bookbagofholding.CONFIG['DOWNLOAD_DIR'], ',')
            if len(templist) and bookbagofholding.DIRECTORY("Download") != templist[0]:
                templist.insert(0, bookbagofholding.DIRECTORY("Download"))
        dirlist = []
                ...
                for book in snatched:
                    ...
                    for fname in downloads:
                        ...
                        if not extn or extn.strip('.') not in skipped_extensions:
                            ...
                            if bookbagofholding.LOGLEVEL & bookbagofholding.log_fuzz:
                                logger.debug("%s%% match %s : %s" % (match, matchtitle, matchname))
                            if match >= bookbagofholding.CONFIG['DLOAD_RATIO']:
                                ...
                                if os.path.isfile(pp_path):
                                    # Check for single file downloads first. Book file in download root.
                                    ...
                                    if is_valid_type(fname, extras=''):
                                        if bookbagofholding.LOGLEVEL & bookbagofholding.log_postprocess:
                                            logger.debug('file [%s] is a valid book' % fname)
                                        if bts_file(download_dir):
                                            ...
                                            if bookbagofholding.CONFIG['DESTINATION_COPY'] or \
                                                    (book['NZBmode'] in ['torrent', 'magnet', 'torznab'] and
                                                     bookbagofholding.CONFIG['KEEP_SEEDING']):
                                ...
                                if os.path.isdir(pp_path):
                                    ...
                                    # processAlternate(pp_path) but that currently only does ebooks, not audio
                    ...
                    if match and match >= bookbagofholding.CONFIG['DLOAD_RATIO']:
                        ...
                        if data:  # it's ebook/audiobook
                            ...
                            if 'windows' in platform.system().lower() and '/' in \
                                    bookbagofholding.CONFIG['EBOOK_DEST_FOLDER']:
                                logger.warn('Please check your EBOOK_DEST_FOLDER setting')
                                bookbagofholding.CONFIG['EBOOK_DEST_FOLDER'] = bookbagofholding.CONFIG[
                                    'EBOOK_DEST_FOLDER'].replace('/', '\\')
                            # Default destination path, should be allowed change per config file.
                            seriesinfo = nameVars(book['BookID'])
                            dest_path = seriesinfo['FolderName']
                            dest_dir = bookbagofholding.DIRECTORY('eBook')
                            if book_type == 'AudioBook' and bookbagofholding.DIRECTORY('Audio'):
                                dest_dir = bookbagofholding.DIRECTORY('Audio')
                            dest_path = os.path.join(dest_dir, dest_path)
                            dest_path = stripspaces(dest_path)
                            global_name = seriesinfo['BookFile']
                        else:
                            # not recognised, maybe deleted
                    ...
                    if success:
                        ...
                        if ignoreclient is False and book['NZBmode'] in ['torrent', 'magnet', 'torznab']:
                            ...
                            if bookbagofholding.CONFIG['KEEP_SEEDING']:
                        ...
                        # books can be re-downloaded from history if needed
                        if book_type == 'eBook':
        ...
        logger.info('%s book%s processed.' % (ppcount, plural(ppcount)))
        ...
        for book in snatched:
            ...
            if book['Status'] == "Aborted":
                abort = True
            elif book['Status'] == "Snatched" and bookbagofholding.CONFIG['TASK_AGE']:
                ...
                hours = int(diff / 3600)
                mins = int(diff / 60)
                if hours >= bookbagofholding.CONFIG['TASK_AGE']:
                    abort = True
            ...
            if abort:
                ...
                if book['BookID'] != 'unknown':
                    ...
                    # use url and status for identifier
                    if book['Status'] == "Snatched":
        ...
        bookbagofholding.POSTPROCESS_UPDATE = False
...
def check_contents(source, downloadid, book_type, title):
    ...
    """
    rejected = ''
    banned_extensions = getList(bookbagofholding.CONFIG['BANNED_EXT'])
    if book_type.lower() == 'ebook':
        maxsize = bookbagofholding.CONFIG['REJECT_MAXSIZE']
        minsize = bookbagofholding.CONFIG['REJECT_MINSIZE']
        filetypes = bookbagofholding.CONFIG['EBOOK_TYPE']
        banwords = bookbagofholding.CONFIG['REJECT_WORDS']
    elif book_type.lower() == 'audiobook':
        maxsize = bookbagofholding.CONFIG['REJECT_MAXAUDIO']
        # REJECT_MINAUDIO is intentionally disabled here because individual
        # audiobook chapters can be quite small (often <1MB for short chapters).
        # The config setting is still used during search result filtering in
        # resultlist.py where full audiobook size is known.
        minsize = 0
        filetypes = bookbagofholding.CONFIG['AUDIOBOOK_TYPE']
        banwords = bookbagofholding.CONFIG['REJECT_AUDIO']
        ...
        if "LL.(" in entry:
            ...
            if not extn or extn.strip('.') not in skipped_extensions:
                ...
                if have_ebook and have_audio:
                    exists = True
                elif have_ebook and not bookbagofholding.SHOW_AUDIO:
                    exists = True
                else:
                    exists = False
...
def getDownloadName(title, source, downloadid):
    ...
    try:
        ...
        if source == 'TRANSMISSION':
            ...
            client = DelugeRPCClient(bookbagofholding.CONFIG['DELUGE_HOST'], int(bookbagofholding.CONFIG['DELUGE_PORT']),
                                     bookbagofholding.CONFIG['DELUGE_USER'], bookbagofholding.CONFIG['DELUGE_PASS'])
            try:
                client.connect()
    ...
    try:
        if Source == "BLACKHOLE":
            ...
            client = DelugeRPCClient(bookbagofholding.CONFIG['DELUGE_HOST'],
                                     int(bookbagofholding.CONFIG['DELUGE_PORT']),
...
def process_book(pp_path=None, bookID=None):
    ...
    try:
        ...
        # Called from "import_alternate" or if we find a "LL.(xxx)" folder that doesn't match a snatched book
        if bookbagofholding.LOGLEVEL & bookbagofholding.log_postprocess:
        ...
        if data:
            cmd = 'SELECT BookID, NZBurl, NZBtitle, NZBprov, AuxInfo FROM wanted WHERE BookID=? and Status="Snatched"'
            ...
            if not is_audio and not is_ebook:
                ...
                if is_audio and not bookbagofholding.SHOW_AUDIO:
                    is_audio = False
                if is_audio:
                    book_type = "AudioBook"
                elif is_ebook:
            ...
            if 'windows' in platform.system().lower() and '/' in bookbagofholding.CONFIG['EBOOK_DEST_FOLDER']:
                logger.warn('Please check your EBOOK_DEST_FOLDER setting')
                bookbagofholding.CONFIG['EBOOK_DEST_FOLDER'] = bookbagofholding.CONFIG['EBOOK_DEST_FOLDER'].replace('/', '\\')
            ...
            if success:
                ...
                if was_snatched:
                    ...
                    # Add to blacklist if BLACKLIST_PROCESSED is enabled
                    if bookbagofholding.CONFIG['BLACKLIST_PROCESSED']:
                        add_to_blacklist(was_snatched[0]['NZBurl'], was_snatched[0]['NZBtitle'],
                                         was_snatched[0]['NZBprov'], bookID,
                                         was_snatched[0].get('AuxInfo'), 'Processed')
                else:
                    controlValueDict = {"BookID": bookID}
                ...
                if not bookbagofholding.CONFIG['DESTINATION_COPY'] and pp_path != dest_dir:
                    if os.path.isdir(pp_path):
                        # calibre might have already deleted it?
                        try:
                            shutil.rmtree(pp_path)
...
def processExtras(dest_file=None, global_name=None, bookid=None, book_type="eBook"):
    ...
    controlValueDict = {"BookID": bookid}
    if book_type == 'AudioBook':
        newValueDict = {"AudioFile": dest_file, "AudioStatus": bookbagofholding.CONFIG['FOUND_STATUS'],
                        "AudioLibrary": now()}
        myDB.upsert("books", newValueDict, controlValueDict)
        if bookbagofholding.CONFIG['AUDIOBOOK_DEST_FILE']:
            if bookbagofholding.CONFIG['IMP_RENAME']:
        ...
        newValueDict = {"Status": bookbagofholding.CONFIG['FOUND_STATUS'], "BookFile": dest_file, "BookLibrary": now()}
        myDB.upsert("books", newValueDict, controlValueDict)
    ...
    if not bookbagofholding.CONFIG['IMP_AUTOADD_BOOKONLY']:
    ...
    if bookbagofholding.CONFIG['IMP_AUTOADD']:
...
def processDestination(pp_path=None, dest_path=None, authorname=None, bookname=None, global_name=None, bookid=None,
                       booktype=None):
    """ Copy/move book and associated files into target directory
        Return True, full_path_to_book  or False, error_message"""

    if not bookname:
        return False, 'No bookname provided'

    booktype = booktype.lower()

    bestmatch = ''
    if booktype == 'ebook' and bookbagofholding.CONFIG['ONE_FORMAT']:
    ...
    else:  # audiobook or multi-format book
        match = False
        for fname in os.listdir(makeBytestr(pp_path)):
            fname = makeUnicode(fname)
    ...
    if len(bookbagofholding.CONFIG['IMP_PREPROCESS']):
        logger.debug("Running PreProcessor: %s %s" % (booktype, pp_path))
        params = [bookbagofholding.CONFIG['IMP_PREPROCESS'], booktype, pp_path]
    ...
    newbookfile = ''
    if booktype == 'ebook' and len(bookbagofholding.CONFIG['IMP_CALIBREDB']):
        dest_dir = bookbagofholding.DIRECTORY('eBook')
        try:
            ...
            if rc:
                return False, 'calibredb rc %s from %s' % (rc, bookbagofholding.CONFIG['IMP_CALIBREDB'])
            ...
            if not bookbagofholding.CONFIG['IMP_AUTOADD_BOOKONLY']:
                ...
                if not data:
                    ...
                    if bookbagofholding.CONFIG['WISHLIST_TAGS']:
        ...
        # we are copying the files ourselves, either it's audiobook or we don't want to use calibre
        if bookbagofholding.LOGLEVEL & bookbagofholding.log_postprocess:
            logger.debug("BookType: %s, calibredb: [%s]" % (booktype, bookbagofholding.CONFIG['IMP_CALIBREDB']))
            ...
            if bestmatch and is_valid_booktype(fname, booktype=booktype) and not fname.endswith(bestmatch):
                ...
                if is_valid_booktype(fname, booktype=booktype) or \
                        ((fname.lower().endswith(".jpg") or fname.lower().endswith(".opf"))
                         and not bookbagofholding.CONFIG['IMP_AUTOADD_BOOKONLY']):
...
def processAutoAdd(src_path=None, booktype='book'):
    ...
    autoadddir = bookbagofholding.CONFIG['IMP_AUTOADD']
    savefiles = bookbagofholding.CONFIG['IMP_AUTOADD_COPY']
    ...
    try:
        ...
        match = False
        if booktype == 'book' and bookbagofholding.CONFIG['ONE_FORMAT']:
            booktype_list = getList(bookbagofholding.CONFIG['EBOOK_TYPE'])
        ...
        for name in names:
            ...
            elif booktype == 'book' and bookbagofholding.CONFIG['IMP_AUTOADD_BOOKONLY'] and not \
...
def processIMG(dest_path=None, bookid=None, bookimg=None, global_name=None, overwrite=False):
    ...
    if bookbagofholding.CONFIG['IMP_AUTOADD_BOOKONLY']:
    ...
    cachefile = os.path.join(bookbagofholding.CACHEDIR, 'book', bookid + '.jpg')
...
def processOPF(dest_path=None, data=None, global_name=None, overwrite=False):
    ...
    if bookbagofholding.CONFIG['OPF_TAGS']:
